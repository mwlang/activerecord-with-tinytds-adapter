...
module ActiveRecord
  ...
  module ConnectionAdapters
    class SQLServerColumn < Column# :nodoc:
      ...
      def initialize(name, default, row, sql_type = nil, identity = false, null = true) # TODO: check ok to remove scale_value = 0
    ...
    class SQLServerAdapter < AbstractAdapter
      ...
      def columns(table_name, name = nil)
        ...
        result.each do |field|
          ...
          if field[:ColType] =~ /numeric|decimal/i
            type = "#{field[:ColType]}(#{field[:numeric_precision] == true ? field[:Length] : '10'},#{field[:numeric_scale]})"
          else
            type = "#{field[:ColType]}(#{field[:Length]})"
          end
          is_identity = field[:IsIdentity] == 1
          is_nullable = field[:IsNullable] == 'YES'
          columns << SQLServerColumn.new(field[:ColName], default, field, type, is_identity, is_nullable)
        end
        columns
      end
      ...
      def quote_utf8_string(value)
        ::Iconv.conv('UTF-8//IGNORE', 'UTF-8', "'#{quote_string(value)}'")
      end
        ...
        case value
          when String, ActiveSupport::Multibyte::Chars
            ...
            if column && column.type == :binary && column.class.respond_to?(:string_to_binary)
              quote_utf8_string(column.class.string_to_binary(value))
            elsif column && [:integer, :float].include?(column.type)
              value = column.type == :integer ? value.to_i : value.to_f
              value.to_s
            else
              quote_utf8_string(value)
            end
